  ...
  container.innerHTML = `
    ...
    <div class="uninstall-layout">
      <div class="card app-list-card">
        ...
        <div class="app-list" id="app-list"><div class="empty-tip">加载中…</div></div>
      </div>
  ...
  detail.innerHTML = `
    <div class="detail-head">
      ...
      <div class="detail-meta">
        ${app.publisher ? `<div><span class="muted">发布者：</span>${escapeHtml(app.publisher)}</div>` : ""}
        ${app.version ? `<div><span class="muted">版本：</span>${escapeHtml(app.version)}</div>` : ""}
        ${app.estimatedSizeKb > 0 ? `<div><span class="muted">已用空间：</span>${formatBytes(app.estimatedSizeKb * 1024)}</div>` : ""}
        ${app.installDate ? `<div><span class="muted">安装日期：</span>${escapeHtml(app.installDate)}</div>` : ""}
        ${app.isUser ? '<div class="muted">安装于：当前用户</div>' : '<div class="muted">安装于：所有用户</div>'}
      </div>
    ...
    <div class="detail-actions">
      <button id="btn-uninstall" class="btn btn-danger" ${app.uninstallString ? "" : "disabled"}>🗑️ 卸载</button>
      <button id="btn-scan-residue" class="btn btn-primary">🔍 扫描残留</button>
    </div>
    <div id="residue-area">
...
function renderResidue(name: string, totalSize: number): void {
  ...
  if (residue.length === 0) {
    area.innerHTML = `<div class="empty-tip">未发现「${escapeHtml(name)}」的残留，卸载得很干净 🎉</div>`;
    return;
  }
