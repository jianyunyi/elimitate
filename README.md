...
- **按修改时间过滤**：可只清理「N 天前」修改的文件（全部 / 7 / 30 / 90 / 180 天），避开正在使用的新文件。
- **删除前占用预检**：被其他程序锁定的文件会被**提前识别并跳过**（提示"N 项被占用"），而不是删除时报错。
- **默认删除到回收站**：一键开关，默认将文件移入回收站（可恢复）；关闭后才是永久删除。
- **清空空目录**：按年龄清理后自动移除已清空的目录。

### 📁 大文件分析（v0.2.0）

- 选择磁盘（自动列出各盘可用空间）并行扫描，找出 **Top N（50/100/200）最大文件**，展示大小与修改日期。
- 实时进度与已扫描文件数，支持**随时停止**（停止后仍显示已发现结果）。
- 默认**跳过系统目录**（Windows / Program Files / ProgramData 等），只聚焦用户数据。
- 对结果可：**打开所在文件夹**、**删除到回收站**、**永久删除**（删除前占用预检 + 确认）。
...
- **默认可恢复**：删除默认移入回收站，可随时恢复；仅明确关闭开关才永久删除。
- **占用预检**：删除前识别被锁定的文件并跳过，提前提示而非事后报错。
- **失败不静默**：权限不足等失败项会收集到错误列表展示，不阻塞其他项目。
- **删除确认**：清理前弹出确认框并显示预计释放空间与删除方式。
- **最小权限**：普通权限即可完成大部分用户级清理；仅系统级操作需要管理员。

## ⚠️ 免责声明

本工具会**永久删除**所选文件（关闭回收站开关时），删除后不可恢复。请在使用前确认所选项目确为垃圾或已卸载软件的残留。作者不对因使用本工具造成的数据丢失负责。

## 🗺️ Roadmap

- [x] 按修改时间过滤（只删 N 天前的文件）
- [x] 删除前占用预检
- [x] 默认删除到回收站（可恢复）
- [x] 大文件/磁盘空间占用分析
- [ ] Firefox 浏览器缓存支持
